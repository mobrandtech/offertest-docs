...
| city | Select one city from this [https://raw.githubusercontent.com/mobrandtech/offertest-docs/master/cities.csv](csv) (it should match the given country or it will be ignored) **(Optional)** **(Consumes 10 Credits)**. If ``city`` is omited the exit city will be automatically selected for the country. Example: ``lasvegas``. |
